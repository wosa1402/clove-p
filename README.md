...
这个流程会在构建时自动选择仓库根目录里的 `warp-linux-amd64` 或 `warp-linux-arm64`，因此容器内可以直接启用 WARP IP 功能。

### 本地构建镜像

仓库已经内置 `warp-linux-amd64` 和 `warp-linux-arm64` 两个 Linux 二进制，本地构建时会自动按目标架构选择：

```bash
docker build -t clove:local .
```

使用 `docker-compose.yml` 时直接构建即可：
...
# 仅用于申请/更新 WARP 身份时访问 Cloudflare API 的前置代理
